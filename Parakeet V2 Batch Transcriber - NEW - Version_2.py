...
3.  For segments that are too long or too wide, it uses precise word-level timestamps to find natural split points.
4.  Splitting logic prioritizes natural boundaries based on punctuation (end of sentence > clause > comma).
5.  Character-width readability constraints prevent visually overloaded subtitles.
6.  Falls back to splitting by word count only as a last resort to meet duration constraints.
7.  Features a smart pre-processing step for extremely long audio files (>60 minutes), splitting them
...
class LongAudioHandler:
    ...
    def find_split_points(self, audio_path: str, duration_sec: float) -> List[int]:
        """
        ...
        by extracting just the small window using FFmpeg.
        Args:
            audio_path (str): The path to the processed WAV file.
        ...
        for i in range(1, num_segments):
            ...
            window_start_sec = window_start_ms / 1000.0
            window_duration_sec = (window_end_ms - window_start_ms) / 1000.0
            
            fd, tmp_window_path = tempfile.mkstemp(suffix=".wav")
            os.close(fd)
            try:
                cmd = ['ffmpeg', '-ss', str(window_start_sec), '-t', str(window_duration_sec),
                       '-i', audio_path, '-c:a', 'pcm_s16le', '-y', '-loglevel', 'error', tmp_window_path]
                subprocess.run(cmd, check=True, timeout=300)
                window_audio = AudioSegment.from_file(tmp_window_path)
            finally:
                if os.path.exists(tmp_window_path):
                    os.unlink(tmp_window_path)
    ...
    def split_audio(self, audio_path: str, temp_dir: str, duration_sec: float) -> List[Dict[str, Any]]:
        """
        Splits a single audio file into multiple WAV chunks using FFmpeg stream copy.
        Args:
            audio_path (str): Path to the source audio file.
            temp_dir (str): Path to the directory for temporary chunk files.
            duration_sec (float): Total duration of the audio in seconds.
        Returns:
        ...
        split_points = self.find_split_points(audio_path, duration_sec)
        if not split_points:
            ...
            return [{'file_path': audio_path, 'start_time': 0, 'end_time': duration_sec, 'duration': duration_sec, 'segment_index': 1, 'is_temp': False}]

        # Perform the split
        segments_info = []
        start_ms = 0
        for i, end_ms in enumerate(split_points + [duration_ms], 1):
            # Extract the audio chunk using FFmpeg stream copy (fast and memory efficient)
            segment_filename = f"segment_{i}.wav"
            segment_path = os.path.join(temp_dir, segment_filename)
            
            start_sec = start_ms / 1000.0
            duration_sec_chunk = (end_ms - start_ms) / 1000.0
            
            cmd = ['ffmpeg', '-ss', str(start_sec), '-t', str(duration_sec_chunk),
                   '-i', audio_path, '-c:a', 'pcm_s16le', '-y', '-loglevel', 'error', segment_path]
            subprocess.run(cmd, check=True, timeout=300)
...
class OptimizedSubtitleProcessor:
    ...
    @staticmethod
    def process_segments(segment_timestamps: List[Dict], word_timestamps: Optional[List[Dict]] = None, max_duration: float = None) -> List[Dict]:
        """
        Processes raw transcription segments to create well-formed subtitles.
        If a segment is too long or too wide (character count), it attempts to split it using word timestamps.
        Args:
        ...
        for segment in segment_timestamps:
            ...
            char_count = len(segment.get('segment', ''))
            needs_split = duration > max_duration or char_count > MAX_CHARACTERS_PER_SEGMENT
            print_debug(f"Processing segment, duration {duration:.1f}s, {char_count} chars, max allowed {max_duration}s/{MAX_CHARACTERS_PER_SEGMENT} chars.")
            # If segment duration and character count are acceptable, keep it as is
            if not needs_split:
                optimized_segments.append(segment)
                print_debug(f"Segment is fine ({duration:.1f}s, {char_count} chars), keeping as is.")
            else:
                # Segment needs splitting (too long or too wide)
                if duration > max_duration:
                    print_warning(f"Found long segment ({duration:.1f}s), which needs splitting.")
                else:
                    print_warning(f"Found wide segment ({char_count} chars), which needs splitting for readability.")
                if word_timestamps:
    ...
    def _split_with_words(segment: Dict, word_timestamps: List[Dict], max_duration: float) -> List[Dict]:
        ...
        Considers both duration and character-width readability constraints.

        Splitting priority (highest to lowest):
        1. Hard max duration exceeded  → force split
        2. Hard max characters exceeded → force split at best punctuation boundary
        3. Sentence-ending punctuation  → split when past ideal duration OR ideal chars
        4. Clause separator punctuation → split when past clause threshold
        5. Soft separator (comma)       → split when past soft threshold
        6. Character readability pressure → split at nearest word boundary
        7. Word-count fallback           → split when MAX_WORDS_PER_SEGMENT reached
        ...
        # Calculate adaptive DURATION split thresholds using IDEAL_SEGMENT_DURATION
        # Cap ideal at 75% of max to stay sensible when max_duration is very small
        ideal = min(IDEAL_SEGMENT_DURATION, max_duration * 0.75)
        clause_threshold = ideal + (max_duration - ideal) * 0.5
        soft_threshold = ideal + (max_duration - ideal) * 0.8

        # Character readability thresholds (read from globals at call time)
        ideal_chars = IDEAL_CHARACTERS_PER_SEGMENT
        max_chars = MAX_CHARACTERS_PER_SEGMENT
        ...
        for i, word_info in enumerate(segment_words):
            ...
            # Compute the current subtitle text and its character count
            current_text = ' '.join([w.get('word', w.get('char', '')) for w in current_segment['words']])
            current_char_count = len(current_text)

            # Calculate how far past the character thresholds we are (0.0 = at ideal, 1.0 = at max)
            if max_chars > ideal_chars:
                char_pressure = max(0.0, (current_char_count - ideal_chars) / (max_chars - ideal_chars))
            else:
                char_pressure = 1.0 if current_char_count > max_chars else 0.0

            # Detect punctuation type for the current word
            word_stripped = word_text.rstrip()
            is_sentence_end = any(word_stripped.endswith(p) for p in SENTENCE_ENDINGS)
            is_clause_sep = any(word_stripped.endswith(p) for p in CLAUSE_SEPARATORS)
            is_soft_sep = any(word_stripped.endswith(p) for p in SOFT_SEPARATORS)

            # Determine if we should split after this word
            should_split = False
            split_before_current = False  # If True, move overflowing word to next segment
            split_reason = ""

            # --- Priority 1: Hard duration limit (highest priority) ---
            if current_duration > max_duration:
                should_split = True
                if len(current_segment['words']) > 1:
                    split_before_current = True
                split_reason = f"Exceeded max duration {current_duration:.1f}s"

            # --- Priority 2: Hard character limit ---
            elif current_char_count > max_chars:
                should_split = True
                if len(current_segment['words']) > 1:
                    split_before_current = True
                split_reason = f"Exceeded max chars {current_char_count}/{max_chars}"

            # --- Priority 3-5: Punctuation-aware splitting with character pressure ---
            # Character pressure progressively lowers the duration thresholds at which
            # punctuation splits are accepted. At char_pressure >= 1.0 (at max_chars),
            # any punctuation triggers an immediate split regardless of duration.
            elif current_duration >= ideal or char_pressure > 0.0:
                # Duration thresholds are lowered by character pressure:
                #   at char_pressure=0 → original thresholds
                #   at char_pressure=1 → all thresholds collapse to 0 (split at any punctuation)
                effective_ideal = ideal * (1.0 - char_pressure)
                effective_clause = clause_threshold * (1.0 - char_pressure)
                effective_soft = soft_threshold * (1.0 - char_pressure)

                if is_sentence_end and current_duration >= effective_ideal:
                    should_split = True
                    if char_pressure > 0:
                        split_reason = f"Sentence ending at {current_duration:.1f}s, {current_char_count} chars (char pressure {char_pressure:.0%})"
                    else:
                        split_reason = f"Sentence ending at {current_duration:.1f}s (past ideal {ideal:.1f}s)"
                elif is_clause_sep and current_duration >= effective_clause:
                    should_split = True
                    split_reason = f"Clause separator at {current_duration:.1f}s, {current_char_count} chars"
                elif is_soft_sep and current_duration >= effective_soft:
                    should_split = True
                    split_reason = f"Soft separator at {current_duration:.1f}s, {current_char_count} chars"

            # --- Priority 6: Character pressure forces split at word boundary ---
            # If we're well past max_chars and no punctuation was found, split anyway
            if not should_split and current_char_count > max_chars:
                should_split = True
                if len(current_segment['words']) > 1:
                    split_before_current = True
                split_reason = f"Character overflow {current_char_count}/{max_chars} (no punctuation available)"

            # --- Priority 7: Word count fallback (last resort) ---
            if not should_split and len(current_segment['words']) >= MAX_WORDS_PER_SEGMENT:
                should_split = True
            ...
            if should_split and len(current_segment['words']) > 0:
                ...
                if split_before_current:
                    # Remove the overflowing word and split before it
                    overflow_word = current_segment['words'].pop()
                    ...
                    # Start next segment with the overflow word
                    current_segment = {'words': [overflow_word], 'start': overflow_word.get('start', 0)}
                else:
                    segment_text = ' '.join([w.get('word', w.get('char', '')) for w in current_segment['words']])
                    split_segments.append({'start': current_segment['start'], 'end': current_segment['words'][-1].get('end', current_segment['start']), 'segment': segment_text.strip()})
    ...
    def _split_by_text(segment: Dict, max_duration: float) -> List[Dict]:
        ...
        if len(sentences) >= num_segments:
            # We have enough sentences to group them (use ceil for even distribution)
            sentences_per_group = max(1, math.ceil(len(sentences) / num_segments))
            split_texts = []
            ...
            words = text.split()
            words_per_segment = max(1, math.ceil(len(words) / num_segments))
            split_texts = []
    ...
    def _split_by_punctuation(text: str) -> List[str]:
        ...
        # Handles: ellipsis (... / …), mixed combos (?! / !!), em/en dashes, and single punctuation
        pattern = r'(\.{2,}|…|[!?！？]{2,}|[.!?。！？]|[;:；：]|[—–])\s*'
        ...
        filtered_sentences = []
        min_sentence_length = 5  # Low threshold for anime (short exclamations like "Run!", "Stop!")
...
def process_audio_with_ffmpeg(audio_path: str, temp_dir: str) -> Tuple[Optional[str], Optional[float]]:
    ...
    Args:
        ...
        temp_dir: Path to the temporary directory for storing processed files.
    """
    try:
        print_info(f"Processing audio file: {os.path.basename(audio_path)}")
        duration_sec = get_audio_info(audio_path)
        if duration_sec is None or duration_sec <= 0:
            print_error(f"Failed to read audio duration using ffprobe for {audio_path}.")
            return None, None
        ...
        print_error(f"Audio processing timed out for {os.path.basename(audio_path)}.")
        return None, None
...
def merge_short_segments(segments: List[Dict], min_duration: float = MIN_SEGMENT_DURATION) -> List[Dict]:
    """Merges subtitle segments shorter than the minimum duration into adjacent segments."""
    ...
    while i < len(segments):
        ...
        if duration < min_duration and (merged or i < len(segments) - 1):
            # Try to merge with the previous segment first
            if merged:
                prev = merged[-1]
                prev['end'] = seg.get('end', prev['end'])
                prev['segment'] = prev.get('segment', '') + ' ' + seg.get('segment', '')
                prev['segment'] = prev['segment'].strip()
                print_debug(f"Merged short segment ({duration:.2f}s) into previous: '{seg.get('segment', '')[:30]}'")
            elif i < len(segments) - 1:
                # No previous segment; merge into the next one
                next_seg = segments[i + 1]
                next_seg['start'] = seg.get('start', next_seg.get('start', 0))
                next_seg['segment'] = seg.get('segment', '') + ' ' + next_seg.get('segment', '')
                next_seg['segment'] = next_seg['segment'].strip()
                print_debug(f"Merged short segment ({duration:.2f}s) into next: '{seg.get('segment', '')[:30]}'")
        else:
            merged.append(seg.copy())
        i += 1
    removed_count = len(segments) - len(merged)
    if removed_count > 0:
        print_info(f"Merged {removed_count} short segment(s) (< {min_duration}s) into adjacent segments.")
    return merged


def ensure_subtitle_gaps(segments: List[Dict], min_gap: float = MIN_SUBTITLE_GAP) -> List[Dict]:
    """Ensures a minimum time gap between consecutive subtitles to prevent visual strobing."""
    if not segments or len(segments) < 2:
        return segments
    ...
    for i in range(1, len(segments)):
        ...
        if gap < min_gap:
            # Trim the end of the previous subtitle to create the gap
            new_end = current.get('start', 0) - min_gap
            # Don't let the previous subtitle become shorter than 100ms
            if new_end > prev.get('start', 0) + 0.1:
                prev['end'] = new_end
                adjustments += 1
            elif gap < 0:
                # Overlapping: at minimum, remove the overlap
                prev['end'] = current.get('start', 0)
                adjustments += 1
        adjusted.append(current)
    if adjustments > 0:
        print_info(f"Adjusted {adjustments} subtitle gap(s) to ensure minimum {min_gap*1000:.0f}ms spacing.")
    return adjusted
...
class BatchTranscriber:
    ...
    def __init__(self, model_name: str = MODEL_NAME, device: str = DEVICE):
        ...
        base_tmp = "/kaggle/working" if os.path.exists("/kaggle/working") else None
        self._temp_dir = tempfile.mkdtemp(prefix="parakeet_tmp_", dir=base_tmp)
        print_debug(f"Created session temporary directory: {self._temp_dir}")
        self.stats = {
            'processed': 0,
            'skipped': 0,
            'failed': 0,
    ...
    def transcribe_segments_separately(self, segments_info: List[Dict], apply_long_audio_settings: bool = True) -> List[Dict]:
        ...
        for seg_info in segments_info:
            ...
            try:
                ...
                try:
                    if seg_info.get('is_temp', True) and os.path.exists(segment_path):
    ...
    def transcribe_single(self, audio_path: str, output_path: str) -> bool:
        ...
        try:
            temp_audio_path, duration_sec = process_audio_with_ffmpeg(audio_path, self._temp_dir)
            ...
            if duration_sec > VERY_LONG_AUDIO_THRESHOLD:
                ...
                segments_info = self.long_audio_handler.split_audio(temp_audio_path, self._temp_dir, duration_sec)
            ...
            # --- Post-process and save subtitle file ---
            optimized_segments = merge_short_segments(optimized_segments)
            optimized_segments = ensure_subtitle_gaps(optimized_segments)
            ...
            self.stats['total_segments'] += len(deduplicated_segments)
            max_seg_dur = max((s.get('end', 0) - s.get('start', 0) for s in deduplicated_segments), default=0)

            print_success(f"Transcription complete: {os.path.basename(audio_path)} ({len(deduplicated_segments)} segments, longest: {max_seg_dur:.1f}s)")
    ...
    def process_directory(self, directory: str, output_dir: str = OUTPUT_DIR, extensions: List[str] = None, skip_existing: bool = True, recursive: bool = False) -> None:
        ...
        # Collect audio files with case-insensitive extension matching
        source_path = Path(directory)
        if recursive:
            all_files = source_path.rglob('*')
        else:
            all_files = source_path.iterdir()
        audio_files = sorted({p for p in all_files if p.is_file() and p.suffix.lower() in extensions_lower})
    ...
    def cleanup(self):
        ...
        if self.model is not None:
            del self.model
            self.model = None
        cleanup_temp_directory(self._temp_dir)
        self._temp_dir = None
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
...
def check_dependencies():
    ...
    # Check for ffmpeg and ffprobe on PATH
    if not shutil.which('ffmpeg'):
        print_error("ffmpeg not found on PATH. Please install FFmpeg and ensure it is accessible.")
        dependencies_ok = False
    else:
        print_debug("ffmpeg found on PATH.")
    if not shutil.which('ffprobe'):
        print_error("ffprobe not found on PATH. Please install FFmpeg (includes ffprobe) and ensure it is accessible.")
        dependencies_ok = False
    else:
        print_debug("ffprobe found on PATH.")

    try:
        # Verify audio processing backend works end-to-end
        test_audio = AudioSegment.silent(duration=100)
        fd, tmp_path = tempfile.mkstemp(suffix=".wav")
        os.close(fd)
        try:
            test_audio.export(tmp_path, format="wav")
        finally:
...
def main():
    ...
    global MAX_SEGMENT_DURATION, MAX_WORDS_PER_SEGMENT, IDEAL_CHARACTERS_PER_SEGMENT, MAX_CHARACTERS_PER_SEGMENT
...
Optimization Strategy:
  ...
  2. For segments that exceed the max duration or character width, it uses precise 'word' timestamps.
  3. Splitting Priority: Duration > Characters > Sentence end > Clause > Comma > Word count.
  4. Character pressure progressively lowers punctuation thresholds to preserve natural flow.
  5. For very long audio (>60 min), it pre-splits the file at silent points to prevent memory issues.

Default Parameters (Optimized for Anime Subtitles):
  - Max Segment Duration: 4 seconds
  - Max Words Limit: 12 words (fallback threshold)
  - Ideal Characters/Line: 42 chars (soft readability target)
  - Max Characters/Line: 64 chars (hard readability limit)
...
Usage Examples:
  ...
  # Adjust segmenting rules for a podcast (wider subtitles allowed)
  %(prog)s ./podcasts --max-segment 10 --max-words 40 --max-chars 80

  # Tighter subtitle width for mobile viewing
  %(prog)s ./anime --ideal-chars 32 --max-chars 48

  # Recursively scan subdirectories
  %(prog)s /path/to/audio --recursive
        """)
    ...
    parser.add_argument("--ideal-chars", type=int, default=IDEAL_CHARACTERS_PER_SEGMENT, help=f"Ideal characters per subtitle line — soft readability target (default: {IDEAL_CHARACTERS_PER_SEGMENT}).")
    parser.add_argument("--max-chars", type=int, default=MAX_CHARACTERS_PER_SEGMENT, help=f"Maximum characters per subtitle line — hard readability limit (default: {MAX_CHARACTERS_PER_SEGMENT}).")
    parser.add_argument("--recursive", action="store_true", help="Recursively scan subdirectories for audio files.")
    ...
    IDEAL_CHARACTERS_PER_SEGMENT = args.ideal_chars
    MAX_CHARACTERS_PER_SEGMENT = args.max_chars
    ...
    # Resolve device without mutating the module-level DEVICE global
    if args.device == "auto":
        device = "cuda" if torch.cuda.is_available() else "cpu"
    else:
        device = args.device
    ...
    print(f"Ideal Chars/Line:      {IDEAL_CHARACTERS_PER_SEGMENT} chars")
    print(f"Max Chars/Line:        {MAX_CHARACTERS_PER_SEGMENT} chars")
    ...
    try:
        if not transcriber.load_model():
            ...
            sys.exit(1)
        transcriber.process_directory(directory=args.directory, output_dir=args.output, extensions=extensions, skip_existing=not args.no_skip, recursive=args.recursive)
