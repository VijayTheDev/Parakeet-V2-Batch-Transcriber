...
class LongAudioHandler:
    ...
    def find_split_points(self, audio_path: str, duration_sec: float) -> List[int]:
        ...
        for i in range(1, num_segments):
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_window:
                cmd = ['ffmpeg', '-ss', str(window_start_sec), '-t', str(window_duration_sec),
                       '-i', audio_path, '-c', 'copy', '-y', '-loglevel', 'error', tmp_window.name]
                subprocess.run(cmd, check=True)
                window_audio = AudioSegment.from_file(tmp_window.name)
    ...
    def split_audio(self, audio_path: str, temp_dir: str, duration_sec: float) -> List[Dict[str, Any]]:
        ...
        for i, end_ms in enumerate(split_points + [duration_ms], 1):
            ...
            cmd = ['ffmpeg', '-ss', str(start_sec), '-t', str(duration_sec_chunk),
                   '-i', audio_path, '-c', 'copy', '-y', '-loglevel', 'error', segment_path]
            subprocess.run(cmd, check=True)
...
class OptimizedSubtitleProcessor:
    ...
    def _split_with_words(segment: Dict, word_timestamps: List[Dict], max_duration: float) -> List[Dict]:
        ...
        # Calculate adaptive split thresholds using IDEAL_SEGMENT_DURATION
        ...
        for i, word_info in enumerate(segment_words):
            ...
            if current_duration > max_duration:
                should_split = True
                # Move the overflowing word to the next segment (unless it's the only word)
                if len(current_segment['words']) > 1:
                    split_before_current = True
                split_reason = f"Exceeded max duration {current_duration:.1f}s"
            # Condition 2: Past ideal duration and a natural break occurs
            elif current_duration >= ideal:
                ...
                elif current_duration >= clause_threshold and any(word_text.rstrip().endswith(punct) for punct in CLAUSE_SEPARATORS):
                    should_split = True
                    split_reason = f"Clause separator at {current_duration:.1f}s"
                elif current_duration >= soft_threshold and any(word_text.rstrip().endswith(punct) for punct in SOFT_SEPARATORS):
                    should_split = True
...
def process_audio_with_ffmpeg(audio_path: str) -> Tuple[Optional[str], Optional[float]]:
    ...
    try:
        ...
        # Create a safe temporary directory using tempfile
        base_tmp = "/kaggle/working" if os.path.exists("/kaggle/working") else None
        if 'SESSION_TEMP_DIR' not in globals():
            global SESSION_TEMP_DIR
            SESSION_TEMP_DIR = tempfile.mkdtemp(prefix="parakeet_tmp_", dir=base_tmp)
            print_debug(f"Created temporary directory: {SESSION_TEMP_DIR}")
...
def cleanup_temp_directory():
    ...
    global SESSION_TEMP_DIR
    if 'SESSION_TEMP_DIR' in globals() and os.path.exists(SESSION_TEMP_DIR):
        try:
            import shutil
            shutil.rmtree(SESSION_TEMP_DIR)
            print_debug(f"Deleted temporary directory: {SESSION_TEMP_DIR}")
            del SESSION_TEMP_DIR
        except Exception as e:
            print_warning(f"Could not delete temporary directory {SESSION_TEMP_DIR}: {str(e)}")
    ...
    def transcribe_single(self, audio_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            if duration_sec > VERY_LONG_AUDIO_THRESHOLD:
                ...
                segments_info = self.long_audio_handler.split_audio(temp_audio_path, temp_dir, duration_sec)
