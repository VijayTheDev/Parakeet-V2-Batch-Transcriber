...
LONG_AUDIO_THRESHOLD = 8 * 60  # 8 minutes in seconds, triggers long audio attention optimization
...
class LongAudioHandler:
    ...
    def find_split_points(self, audio: AudioSegment) -> List[int]:
        ...
        for i in range(1, num_segments):
            ...
            if silence_ranges:
                # Find the best silent passage: prefer the one closest to the target split point
                # among all sufficiently long silences (all are >= min_silence_len by definition)
                best_silence = min(silence_ranges, key=lambda s: abs(
                    window_start_ms + (s[0] + s[1]) / 2 - target_time_ms
                ))
    ...
    def split_audio(self, audio_path: str, temp_dir: str, already_converted: bool = False) -> List[Dict[str, Any]]:
        ...
        Uses pydub for silence detection, then ffmpeg for memory-efficient splitting.
        Args:
            audio_path (str): Path to the source audio file.
            temp_dir (str): Path to the directory for temporary chunk files.
            already_converted (bool): Ignored; ffmpeg always outputs 16kHz mono.
        Returns:
        ...
        # Load audio for silence-based split point detection
        ...
        # Release audio memory before performing the actual splits
        del audio
        gc.collect()
        ...
        # Use ffmpeg for the actual splitting (memory-efficient: no full audio in Python memory)
        segments_info = []
        start_ms = 0
        for i, end_ms in enumerate(split_points + [duration_ms], 1):
            ...
            cmd = ['ffmpeg', '-i', audio_path, '-ss', str(start_sec), '-t', str(segment_duration_sec),
                   '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', '-y', '-loglevel', 'error', segment_path]
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='ignore', timeout=600)
                if result.returncode != 0:
                    print_error(f"ffmpeg failed to extract segment {i}: {result.stderr}")
                    continue
            except subprocess.TimeoutExpired:
                print_error(f"ffmpeg timed out extracting segment {i}")
                continue
...
class OptimizedSubtitleProcessor:
    ...
    @staticmethod
    def process_segments(segment_timestamps: List[Dict], word_timestamps: Optional[List[Dict]] = None, max_duration: float = MAX_SEGMENT_DURATION, max_words: int = MAX_WORDS_PER_SEGMENT, min_duration: float = MIN_SEGMENT_DURATION) -> Tuple[List[Dict], Dict[str, int]]:
        """
        ...
        Short segments are merged to meet min_duration.
        Args:
            ...
            min_duration: The minimum allowed duration; shorter segments are merged.
        Returns:
            A tuple of (optimized subtitle segments, split statistics dict).
        """
        ...
        optimized_segments = []
        split_stats = {'split_segments': 0, 'word_level_splits': 0}
        for segment in segment_timestamps:
            ...
            if duration <= max_duration:
                ...
                if word_timestamps:
                    ...
                    split_stats['word_level_splits'] += 1
                else:
                ...
                split_stats['split_segments'] += len(split_segments)
                print_debug(f"Split into {len(split_segments)} segments.")

        # Merge segments that are too short to be readable
        optimized_segments = OptimizedSubtitleProcessor._merge_short_segments(optimized_segments, max_duration, min_duration)
        return optimized_segments, split_stats

    @staticmethod
    def _merge_short_segments(segments: List[Dict], max_duration: float, min_duration: float = MIN_SEGMENT_DURATION) -> List[Dict]:
        """
        Merges subtitle segments shorter than min_duration with adjacent segments,
        while respecting max_duration. This prevents subtitle flicker from very
        short lines (common in anime). Re-checks merged results to handle
        consecutive short segments.
        Args:
        ...
        while i < len(segments):
            ...
            # Keep merging with the next segment while current is too short
            while current_duration < min_duration and i < len(segments):
                next_seg = segments[i]
                ...
                if combined_duration <= max_duration:
                    ...
                    break  # Can't merge without exceeding max_duration

            merged.append(current)

        if merge_count > 0:
            print_info(f"Merged {merge_count} short segments (< {min_duration}s) with adjacent segments.")
        return merged
    ...
    def _split_with_words(segment: Dict, word_timestamps: List[Dict], max_duration: float, max_words: int = MAX_WORDS_PER_SEGMENT, word_starts: Optional[List[float]] = None) -> List[Dict]:
        ...
        segment_end = segment.get('end', segment.get('start', 0))
    ...
    def _split_by_punctuation(text: str) -> List[str]:
        ...
        Returns:
            A list of sentences.
        """
        # Regex to split by punctuation while keeping the punctuation (handles multiples like !!!)
        pattern = r'([.!?;:。！？；：]+)\s*'
        ...
        for i, part in enumerate(parts):
            if i % 2 == 0:
                ...
                current = ""
        if current.strip():
            sentences.append(current.strip())  # Add the last part if it has no trailing punctuation
...
def validate_subtitle_timestamps(segments: List[Dict], min_gap: float = 0.05) -> List[Dict]:
    ...
    for i in range(1, len(segments)):
        ...
        # Merge degenerate segments (start at or before previous start) into the previous segment
        # to avoid losing transcribed content
        if curr_start <= prev_start:
            prev_text = prev.get('segment', '').strip()
            curr_text = current.get('segment', '').strip()
            if curr_text and curr_text != prev_text:
                fixed[-1]['segment'] = f"{prev_text} {curr_text}".strip()
                # Extend the previous segment's end time if the degenerate segment ends later
                curr_end = current.get('end', 0)
                if curr_end > prev_end:
                    fixed[-1]['end'] = curr_end
                print_warning(f"Merged degenerate segment at {curr_start:.3f}s into previous segment (at or before previous start {prev_start:.3f}s)")
            continue

        # Fix overlap or insufficient gap between segments safely
        if curr_start - prev_end < min_gap:
            adjusted_end = curr_start - min_gap
            # Ensure end is always after start and within valid range
            adjusted_end = max(prev_start + 0.001, adjusted_end)
            adjusted_end = min(adjusted_end, curr_start - 0.001)
            if adjusted_end > prev_start:
                fixed[-1]['end'] = adjusted_end
                fixes_count += 1
                print_debug(f"Adjusted segment end at {curr_start:.3f}s to fix overlap/gap: {prev_end:.3f}s -> {fixed[-1]['end']:.3f}s")
...
class BatchTranscriber:
    ...
    def __init__(self, model_name: str = MODEL_NAME, device: str = DEVICE,
                 ...
                 max_words_per_segment: int = MAX_WORDS_PER_SEGMENT):
    ...
    def transcribe_segments_separately(self, segments_info: List[Dict], apply_long_audio_settings: bool = True) -> List[Dict]:
        ...
        for seg_info in segments_info:
            ...
            try:
                ...
                with torch.no_grad():
                    hypotheses = self.model.transcribe([segment_path], timestamps=True, return_hypotheses=True)
    ...
    def transcribe_single(self, audio_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            if duration_sec > VERY_LONG_AUDIO_THRESHOLD:
                ...
                optimized_segments, split_stats = self.subtitle_processor.process_segments(merged_segments, merged_words if merged_words else None, self.max_segment_duration, self.max_words_per_segment, self.min_segment_duration)
                self.stats['split_segments'] += split_stats['split_segments']
                self.stats['word_level_splits'] += split_stats['word_level_splits']
            else:
                ...
                try:
                    ...
                    with torch.no_grad():
                        hypotheses = self.model.transcribe([temp_audio_path], timestamps=True, return_hypotheses=True)
                    ...
                    optimized_segments, split_stats = self.subtitle_processor.process_segments(segment_timestamps, word_timestamps, self.max_segment_duration, self.max_words_per_segment, self.min_segment_duration)
                    self.stats['split_segments'] += split_stats['split_segments']
                    self.stats['word_level_splits'] += split_stats['word_level_splits']
    ...
    def process_directory(self, directory: str, output_dir: str = OUTPUT_DIR, extensions: List[str] = None, skip_existing: bool = True, recursive: bool = False) -> None:
        ...
        scan_iter = Path(directory).rglob('*') if recursive else Path(directory).iterdir()
        for f in scan_iter:
        ...
        if not audio_files:
            print_warning(f"No audio files found in {directory}{' (recursive)' if recursive else ''}")
            return

        total_files = len(audio_files)
        print_info(f"Found {total_files} audio files to process.")

        # Probe total duration for progress estimation
        total_estimated_duration = 0
        for audio_file in audio_files:
            dur = get_audio_info(str(audio_file))
            if dur:
                total_estimated_duration += dur
        if total_estimated_duration > 0:
            print_info(f"Total estimated audio duration: {format_time(total_estimated_duration)}")
        print("=" * 60)
...
def check_dependencies():
    ...
    try:
        import nemo
        print_debug("NeMo is installed.")
    except ImportError:
        print_error("NeMo not found. Please install it: pip install nemo_toolkit[asr]")
    ...
    try:
        ...
        print_error(f"Audio processing backend is not working: {str(e)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch transcribe audio files to SRT subtitles (with smart splitting for long audio).", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=f"""
Optimization Strategy:
  ...
  4. For very long audio (>{VERY_LONG_AUDIO_THRESHOLD/60:.0f} min), it pre-splits the file at silent points to prevent memory issues.

Default Parameters (Tuned for Anime):
  - Max Segment Duration: {MAX_SEGMENT_DURATION} seconds
  - Min Segment Duration: {MIN_SEGMENT_DURATION} seconds
  - Max Words Limit: {MAX_WORDS_PER_SEGMENT} words (fallback threshold)
  - Long Audio Threshold: {LONG_AUDIO_THRESHOLD/60:.0f} minutes (applies model optimization)
  - Very Long Audio Threshold: {VERY_LONG_AUDIO_THRESHOLD/60:.0f} minutes (triggers pre-splitting)
...
Usage Examples:
  ...
  # Process audio files in subdirectories
  %(prog)s /path/to/series --recursive
        """)
    ...
    parser.add_argument("-r", "--recursive", action="store_true", help="Recursively scan subdirectories for audio files.")
    ...
    print(f"Max Segment Duration:  {args.max_segment} seconds")
    print(f"Min Segment Duration:  {args.min_segment} seconds")
    print(f"Max Words/Segment:     {args.max_words} words")
